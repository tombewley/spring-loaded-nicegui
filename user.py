...
from settings import DESCOPE_PROJECT_ID, DESCOPE_FLOW
...
def login_form() -> ui.element:
    ...
    with ui.card().classes("w-96 mx-auto"):
        return (
            ...
            .props(f'project-id="{DESCOPE_PROJECT_ID}" flow-id={DESCOPE_FLOW}')
            .on(
                "success",
    ...
    def __call__(self, func: Callable[..., Any]) -> Callable[..., Any]:
        async def content():
            ...
            ui.add_body_html(
                f"""
                <script>
                    const sdk = Descope({{ projectId: '{DESCOPE_PROJECT_ID}', persistTokens: true, autoRefresh: true }});
...
def login_page(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    return page(page.LOGIN_PATH, title="Log in")(func)
